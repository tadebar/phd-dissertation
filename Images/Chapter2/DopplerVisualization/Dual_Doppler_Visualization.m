...
% This version (i.e., with the Dual in front of the title) produces two
% subplots, one with the actual Doppler data, the other with the
% segmentated Doppler data/points for illustration
% -------------------------------------------------------------------------
%% Load Data
clear all; close all;
% define constants
...
ta = title('(a)','FontName','Times New Roman','FontSize',fsz);
set(ta,'Position',[-750,750,-300]);
...
drawnow; hold off; axis equal; view(-135,25); set(gca,'ZDir','reverse');
grid off; set(gca,'FontName','Times New Roman','FontSize',fsz);
axis([-80 80 -60 60 20 120 ])
xlab = xlabel('x dir. (mm)'); zlab = ylabel('z dir. (mm)'); zlabel('y dir. (mm)');
set(xlab,'position',[-710 780 -330]);
set(zlab,'position',[-790 690 -330]);

clear Doppler_SC

subplot(1,2,2);
tb = title('(b)','FontName','Times New Roman','FontSize',fsz);
set(tb,'Position',[-750,750,-300]);
% for slice = 1:doppInt:hdr.fpV
...
% Plot the SC points
scatter3(pts_SC(:,1),pts_SC(:,3),pts_SC(:,2),'r+');
